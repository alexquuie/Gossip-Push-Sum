...
object Project2Bonus {
  ...
  case object BuzzAlarmNetworkMonitor
  case object WakeUpGossip
  case object WakeUpPushSum
  case object WakeUpNetworkMonitor
  case object NodeDead
  case class StopWorking(algo: String)
  ...
  def main(args: Array[String]) {
    ...
    if (args.length < 3) {
      ...
      try {
        ...
        faultPercent = args(3).toDouble
              
        if (numNodes <= 1 || !(faultPercent >= 0 && faultPercent <= 100)) {
          isValidInputArgs = false
        }
      } catch {
        case ex: Exception => {
    ...
    var networkMonitor = system.actorOf(Props(classOf[NetworkMonitor], numNodes, topology, algo, faultPercent), "networkMonitor")
    networkMonitor ! StartMonitor
  }

  class NetworkMonitor(pNumNodes: Int, pTopology: String, pAlgo: String, pFaultPercent: Double) extends Actor {
    ...
    val faultPercent = pFaultPercent
    var alarmClock: ActorRef = null
    var faulScheduler: Cancellable = null
    var totalFaultNodes = 0
    var numNodesFaulted = 0
    ...
    def receive = {
      case StartMonitor => {
        ...
        totalFaultNodes = (Math.ceil((numNodes * faultPercent)/100)).toInt
        alarmClock = context.actorOf(Props(classOf[AlarmClock]), "alarm")
        
        val system = context.system
        import system.dispatcher
        faulScheduler = system.scheduler.schedule(Duration(1000, TimeUnit.MILLISECONDS), Duration(1000, TimeUnit.MILLISECONDS), alarmClock, BuzzAlarmNetworkMonitor)
      }
      ...
      case NodeDead => {
        ...
        if (nodes.size() == 0) {
          ...
          faulScheduler.cancel()
          context.system.shutdown()
        }
      }
      
      case WakeUpNetworkMonitor => {
        if(numNodesFaulted < totalFaultNodes && nodes.size() > 0){
          numNodesConverged += 1
          var randNode = getRandomNode(nodes)
          randNode ! StopWorking(algo)
        }else{
          faulScheduler.cancel()
        }
      }
  ...
  class Node(pNodeId: String, pS: Double, pW: Double) extends Actor {
    ...
    def receive = {
      ...
      case StopWorking(algo) => {
        algo match {
          case ALGORITHM_GOSSIP => {
            stopCount = MAX_RUMOUR_COUNT + 1
          }
  ...
  class AlarmClock extends Actor {
    def receive = {
      ...
      case BuzzAlarmNetworkMonitor => {
        sender ! WakeUpNetworkMonitor
      }
    }
  }
